...
# --------------------------
# FUNCTION TO SHOW THE INSTANCE INFROMATION
# --------------------------
def mostrar_info_archivo(nodes, vehicle_capacity):
    """Show information of he loaded file"""
    ...
    print("\n" + "=" * 50)
    print("Information of the loaded file")
    print("=" * 50)
    print(f"- Vehicle capacity: {vehicle_capacity}")
    print(f"- Number of customers: {num_clientes}")
    print(f"- Total demand: {demanda_total}")
    print(f"- minimum demand: {min_demanda}")
    print(f"- Maximum demand: {max_demanda}")
    print("- Depot (coordinates):", f"({nodes[0]['x']}, {nodes[0]['y']})")
    print("=" * 50 + "\n")
...
def print_solution(routes, dist_matrix, nodes, vehicle_capacity):
    ...
    for i, route in enumerate(routes, 1):
        ...
        print(f"\nRoute {i}:")
        print(f"- Secuence: {route}")
        print(f"- Distance: {route_dist:.2f}")
        print(f"- Demand: {demand}/{vehicle_capacity}")
    print("\n" + "=" * 50)
    print(f"Final resume ")
    print(f"- Total distance: {total_dist:.2f}")
    print(f"- Used vehicles: {len(routes)}")
    print(
        f"- Average capacity used: {sum(sum(nodes[node]['demand'] for node in route[1:-1]) for route in routes) / len(routes):.1f}/{vehicle_capacity}")
    print("=" * 50)
...
def main():
    while True:
        print("=== Algoritm to minimize the traveled distance===")
        print("Select your instance\n")

        file_path = select_file()
        if not file_path:
            print("There are no files selected. Exiting...")
            break
        ...
        continuar = input("\nWould you like to process another file? (1 = yes, Other number = No): ")
        if continuar != "1":
